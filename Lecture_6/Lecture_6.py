...
def PCA_test():
    #initialise data with PCA decomposition
    ...
    #create dictionary with means and covariances
    ...
    for i in range(10):
        ...
        likelihood_pca = func(test_PCA, tempval[0], tempval[1])
        posterior_pca = [x * tempval[2] for x in likelihood_pca[0]]
        templist.append(posterior_pca)

    #classify
    pca_array = np.array(templist)
    clfsy = np.argmax(pca_array, axis = 0)
...
def LDA_test():
    ...
    templist = []
    #There have been attempts to clarify variable names but those attempts confusingly result in memory allocation errors.
    for i in range(10):
        tempstr = str(i)
        tempval = mean_cov_priors_dict[tempstr]
        likelihood_lda = func(test_LDA, tempval[0], tempval[1])
        posterior_lda = [x * tempval[2] for x in likelihood_lda[0]]
        templist.append(posterior_lda)
    #classify
    lda_array = np.array(templist)
    clfsy = np.argmax(lda_array, axis = 0)
